         ...
         [(= (length sl) 4)
          (case (s-exp->symbol (first sl))
            ...
            [(ifzero) (ifZeroC 
                       ...
                       (parse (third sl))
                       (parse (fourth sl)))]
            [else (error 'parse "invalid keyword !")]
            )]
         [(= (length sl) 3)
          (case (s-exp->symbol (first sl))
...
;; Example Environment.
(define Bultin-Environment
  (list
   (bind 'x (numV 5))
   (bind 'y (numV 6))
   (bind 'z (numV 7)) ))
;   (bind 'sqr (lamC 'x (parse '(* x x))))
;   (bind 'sub1 (parse '(lambda! x (+ x -1))))
;   (bind 'neg (parse '(lambda! x (* x -1))))
;   (bind 'double (parse '(lambda! x (+ x x))))
;   (bind 'quadruple (parse '(lambda! x (double (double x)))))
;   (bind 'const5 (parse (number->s-exp 5)))
;   (bind 'factorial (parse 
;                     '(lambda! n (ifzero n 1
;                                         (* n (factorial (sub1 n)))))))
;   (bind 'fibonacci (parse 
;                     '(lambda! n (ifzero n 1
;                                         (ifzero (- n 1) 1
;                                                 (ifzero (- n 2) 1
;                                                         (+ (fibonacci (- n 1))
...
;; Tests:
; <<<<<< ADD TESTS >>>>>>>>
(test (lookup 'x Bultin-Environment) (numV 5))
(test (lookup 'y Bultin-Environment) (numV 6))
(test (lookup 'z Bultin-Environment) (numV 7))
(test/exn (lookup 'w Bultin-Environment) "lookup: name not found")
...
; <<<<<< ADD TESTS >>>>>>>>
(test 
 (interp (parse '(+ 10 ((defun const5 _ 5) 10))) mt-env) (numV 15))

(test/exn 
 (interp (parse '((defun f1 x ((defun f2 y (+ x y)) 4)) 3)) mt-env) "name not found")
