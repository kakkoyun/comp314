...
;; Tests :
(test (BinaryOperationWrapper + (numV 3)(numV 4))(numV 7))
(test (BinaryOperationWrapper * (numV 3)(numV 4))(numV 12))
(test (BinaryOperationWrapper expt (numV 3)(numV 4))(numV 81))
...
(define (eval (sexp : s-expression)) : number
  (numV-n (interp (parse sexp) mt-env)))
...
;; Just does not work because of we couldn't not use recursive calls, 
;; - since we use anonymous functions and we do not have,
;; - function definitions with a namespace.
